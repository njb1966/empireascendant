...
For LLM-assisted development, this plan is not the only authority. Use the project control
...
4. This file for intended Go rewrite behavior.
5. `CONFIG.DOM` for original startup defaults.
6. `DOMINION.PAS` and `SETUP.PAS` for original data model, config parsing, player creation,
   menus, and historical intent.
7. `INTERDOOR_DESIGN_GUIDE.md` for broad InterDoor context only.
8. Outside-folder InterDoor material only for current protocol/interface requirements, never as Empire Ascendant design authority.

`PHASE_PLAN.md` expands the phase breakdown below into LLM-sized implementation slices.
`STANDUP_TO_COMPLETION.md` and `LLM_SESSION_STEPS.md` define the session workflow.
...
When original startup defaults matter, prefer `CONFIG.DOM` over the introductory comment in
`DOMINION.PAS`; the config file is what the Pascal setup reader parses.

---

## Architecture

**Where it lives:** This folder is the Empire Ascendant project root and source of truth:

```
games/empireascendant/
```

Empire Ascendant should be developed inside this folder and its subfolders. It should be able
to produce an independently runnable `interdoor-dominion` binary with its own SQLite database,
configuration, terminal/session flow, and eventual InterDoor federation registration.

Do not derive the implementation structure from any sample game. Other InterDoor code may be
read only to confirm current protocol or interface requirements. Those references constrain
compatibility; they do not define Empire Ascendant's architecture, scope, mechanics, or depth.

The exact local Go package layout is a D1 planning decision. Prefer a clear folder-local
structure that keeps Empire Ascendant understandable as its own project.

**Binary:** `interdoor-dominion` unless a later local decision changes it.
...
Same InterDoor hub queue concept. Attacker submits to hub; victim node drains on next
...
### D1 — Walking skeleton
- local Empire Ascendant Go project structure under this folder
- `interdoor-dominion` binary, port `:2324`, own DB, own config
...
- Daily turn reset
- local terminal/SSH connect, login, works end-to-end
...
**Host:** contabo4 (`147.93.129.235`) — iwebalmanac.net VPS. Chosen because it is the
emptiest node in the fleet (148GB disk at 3% used, 8GB RAM, Debian 13) and has no port
conflicts with the game's SSH listener. Low-traffic IWA sites cohabitate without issue.

**Hub:** InterDOOR federation hub runs on contabo2 (`hub.interdoor.net`). Empire Ascendant registers with it over public HTTPS — no special routing required.

**Game node ports on contabo4:**

| Binary | Port | Notes |
|---|---|---|
| `interdoor-dominion` (Empire Ascendant) | 2324 | Free on contabo4 |

Port 2324 needs to be opened in contabo4's firewall at deploy time. Current open ports on
contabo4 are 22, 80, 443, 1965 — no conflicts.

**Binary path:** `/usr/local/bin/interdoor-dominion` (matches hub convention on contabo2).
**DB path:** `/var/lib/interdoor-dominion/dominion.db`
**Systemd unit:** `interdoor-dominion.service`
